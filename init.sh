...
if [[ -L "$PROJECT_MARKER" ]] || ! { [[ -f "$PROJECT_MARKER" ]] && grep -qx "$EXPECTED_ID" "$PROJECT_MARKER"; }; then
  err "Could not find valid project marker. Exiting."
  exit 1
fi
...
# Library imports
source "$SCRIPT_DIR/lib/utils/init_lib.sh"
source "$SCRIPT_DIR/lib/utils/debs_lib.sh"
source "$SCRIPT_DIR/lib/utils/menu_functions.sh"
